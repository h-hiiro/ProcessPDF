...
int main(int argc, char** argv){
	...
	strcpy(header, "Page,Left,Bottom,Right,Top,FieldName,FieldType,FieldValue\r\n");
	file.write(header, strlen(header));
	char* buffer=new char[1024];
	char* format=new char[128];
	strcpy(format, "%d,%.2f,%.2f,%.2f,%.2f,%s,%s,");
				...
				for(j=0; j<numAnnots; j++){
				  ...
				  error=false;
					...
					if(unsignedstrcmp(FieldType, "Tx")){
						...
						sprintf(buffer, format, i+1, RectValue[0], RectValue[1], RectValue[2], RectValue[3],
